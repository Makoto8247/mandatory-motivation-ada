...
package body Motivational_Messages is
   ...
   function Get_Random_Message return Unbounded_String is
      JSON_Root         : Read_Result;
      Messages          : JSON_Array;
      Messages_Length   : Natural;
   ...
   begin
      ...
      case JSON_Root.Success is
         when True =>
            Messages := JSON_Root.Value.Get ("messages");
      ...
      -- ランダムに1つ選択
      Rnd_Num := Random_Natural.Random (Gen, 1, Messages_Length);

      -- JSON配列から文字列を取得
      Message_Value := Get (Messages, Rnd_Num);
   ...
   exception
      when NOT_GET_FILE =>
         ...
         return To_Unbounded_String ("Failed to get JSON file");
      when others =>
